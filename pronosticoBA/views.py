...
def procesamiento(request):
  ...
  return render(request,'../templates/pronosticoBA/pronosticoBA.html',{'data':data})

def realizarPronosticoBosque(request):
  if request.method=='POST':
    ...
    if not arboles and not depth and not minNode and not minLeaf:
      pronosticoAD= RandomForestRegressor(random_state=0)
    else:
      pronosticoAD= RandomForestRegressor(n_estimators=int(arboles),max_depth=int(depth),min_samples_split=int(minNode),min_samples_leaf=int(minLeaf))
    ...
    if not arboles:
      estimador=pronosticoAD.estimators_[100]
    else:
      numAr=random.randint(1,int(arboles))
      estimador=pronosticoAD.estimators_[numAr]
